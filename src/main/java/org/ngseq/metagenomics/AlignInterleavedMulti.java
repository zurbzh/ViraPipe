 ...
 * Usage:
 * //ON SINGLE NODE
 * spark-submit --master local[${NUM_EXECUTORS}] --executor-memory 30g --class
 * org.ngseq.metagenomics.AlignInterleavedMulti metagenomics-0.9-jar-with-dependencies.jar -in
 * ${OUTPUT_PATH}/${PROJECT_NAME}_interleaved -out ${OUTPUT_PATH}/${PROJECT_NAME}_aligned -ref ${REF_INDEX_IN_LOCAL_FS}
 * //ON YARN CLUSTER
 ...
 * --executor-memory 30g --conf spark.yarn.executor.memoryOverhead=10000 --class
 * org.ngseq.metagenomics.AlignInterleavedMulti virapipe-0.9-jar-with-dependencies.jar -in
 * ${OUTPUT_PATH}/${PROJECT_NAME}_interleaved -out ${OUTPUT_PATH}/${PROJECT_NAME}_aligned -ref ${REF_INDEX_IN_LOCAL_FS}
 *
 */
public class AlignInterleavedMulti {
  ...
  public static void main(String[] args) throws IOException {
    ...
    Arrays.asList(dirs).forEach(dir -> {
      ...
      JavaPairRDD<Text, SequencedFragment> fastqRDD = sc.newAPIHadoopFile(dir.getPath().toString(),
          FastqInputFormat.class, Text.class, SequencedFragment.class, sc.hadoopConfiguration());
      ...
      JavaPairRDD<Text, SequencedFragment> alignmentRDD = fastqRDD.mapPartitionsToPair(split -> {
        ...
        try {
          ...
          List<ShortRead> L1 = new ArrayList<>();
          List<ShortRead> L2 = new ArrayList<>();
          ...
          while (split.hasNext()) {
            ...
            if (split.hasNext()) {
              ...
              if (key.equalsIgnoreCase(key2)) {
                L1.
                    add(new ShortRead(key, sf.getSequence().toString().getBytes(), sf.getQuality().toString().getBytes()));
                L2.add(new ShortRead(key2, sf2.getSequence().toString().getBytes(), sf2.getQuality().toString().
                    getBytes()));
              } else {
                split.next();
              }
          ...
          if (aligns != null) {
            ...
            ArrayList<Tuple2<Text, SequencedFragment>> filtered = new ArrayList<>();
      ...
      if (numpartitions != 0) {
        alignmentRDD.count();
        alignmentRDD.coalesce(numpartitions).saveAsNewAPIHadoopFile(outDir + "/" + output, Text.class,
            SequencedFragment.class, FastqOutputFormat.class, sc.hadoopConfiguration());
      } else {
        alignmentRDD.saveAsNewAPIHadoopFile(outDir + "/" + output, Text.class, SequencedFragment.class,
            FastqOutputFormat.class, sc.hadoopConfiguration());
      }
    });
    sc.stop();
